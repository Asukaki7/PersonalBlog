...
class Post(models.Model):
    ...
    desc = models.TextField(max_length=200, blank=True, verbose_name="文章描述")
...
class Sidebar(models.Model):
    ...
    content = models.CharField(max_length=500, blank=True, default='', verbose_name="内容",
                               help_text="如果设置的不是HTML类型，可为空")  # 这个字段专门给html类型的 其他为空
    sort = models.PositiveIntegerField(default=1, verbose_name="排序", help_text='序号越大越靠前')
    status = models.PositiveIntegerField(default=2, choices=STATUS, verbose_name="状态")  # 隐藏显示状态
    ...
    @classmethod  # 类方法展示器，这个就变成了这个类的一个方法可以调用
    def get_sidebar(cls):
    ...
    def get_content(self):
        if self.display_type == 1:
            ...
            return render_to_string('blog/search.html', context=context)
        elif self.display_type == 2:
            context = {

            }
            return render_to_string('blog/new_post.html', context=context)
        elif self.display_type == 3:
            context = {

            }
            return render_to_string('blog/hot_post.html', context=context)
        elif self.display_type == 4:
            context = {

            }
            return render_to_string('blog/comment.html', context=context)
        elif self.display_type == 5: #文章归档
            context = {

            }
            return render_to_string('blog/archives.html', context=context)
